...
if ('each' in opt) {                       // one trimmed PNG per keep contour (multi-region masks)
  ...
  for (let i = 0; i < keeps.length; i++) {
    const file = `${slug(keeps[i].name, i)}.png`;
    await cut([keeps[i]], resolve(dir, file), true);
    console.log(`  ${file}`);
  }
  console.log(`cut ${keeps.length} region(s) of ${name} @ ${W}x${H} -> ${dir.replace(ROOT + '/', '')}/`);
} else {
  const out = resolve(ROOT, opt.out || `asset-review/cut-${name}.png`);
